...
namespace JN.K3.YDL.App.ServicePlugIn.SCM.PurMRB
{
    [Description("采购退货单锁库插件专用")]
        ...
        public override void BeginOperationTransaction(BeginOperationTransactionArgs e)
        {
            base.BeginOperationTransaction(e);
             ...
             string sql = string.Format(@" SELECT t.FBILLNO, t.FID,t1.FENTRYID,t1.FSEQ, FSTOCKORGID AS FSTOCKORGID,	FKEEPERTYPEID AS FKEEPERTYPEID,	FKEEPERID  AS FKEEPERID,
	                            ...
	                            FBASEUNITQTY AS FBASEQTY,	FBASEUNITID AS FBASEUNITID,	FAUXUNITQTY AS FSECQTY,	FAUXUNITID AS FSECUNITID,FUNITID AS FUNITID,t1.FRMREALQTY AS FQTY 
	                             FROM T_PUR_MRB t INNER JOIN T_PUR_MRBENTRY t1 ON t.FID=t1.FID
                                inner join T_BD_MATERIAL TM on t1.FMATERIALID=TM.FMATERIALID 
                                WHERE  FBusinessType  <> 'ZCCG' and  FStockStatusId <> FReceiveStockStatusId
                                 and t.FID in ({0})", string.Join(",", lstPkIds));
             Common.LockCommon.LockInventory(this.Context, this.OperationResult, sql, "PUR_MRB");
