        ...
        public override void BeginOperationTransaction(BeginOperationTransactionArgs e)
        {
            base.BeginOperationTransaction(e);
             ...
             string sql = string.Format(@"select TKE.FENTRYID as Id,TKE.FSUPPLYINTERID as FInvDetailID,TKH.FDEMANDENTRYID as BillDetailID,TKE.FBASEQTY as FBASEQTY,TKE.FSECQTY as FSECQTY 
                                        ...
                                        inner join T_PUR_MRBENTRY BP on TKH.FDEMANDENTRYID = BP.FEntryID 
                                        inner join T_PUR_MRB TB on BP.FID = TB.FID and TKH.FDEMANDBILLNO = TB.FBILLNO
                                        where  BP.FID in({0}) AND TKH.FSRCFORMID='PUR_MRB' ", string.Join(",", lstPkIds));
             DynamicObjectCollection dyUnLockInfo= DBUtils.ExecuteDynamicObject(this.Context, sql);
             Common.LockCommon.UnLockInventory(this.Context, dyUnLockInfo, "PUR_MRB");
