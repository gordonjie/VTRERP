...
{
    [Description("内蒙采购流程-采购订单至采购入库单-单据转换插件")]
    public class PurOrderToInStockService : POConvertBaseService
    {
        public override void DoExecute(Kingdee.BOS.Core.DynamicForm.IDynamicFormView targetView)
        {
            DynamicObjectCollection dataEntrys = targetView.Model.DataObject["InStockEntry"] as DynamicObjectCollection;
            ...
            for (int row = dataEntrys.Count - 1; row >= 0; row--)
            {
                ...
                //给默认携带行赋值
                targetView.Model.SetValue("FRealQty", srcOutData[0]["FREALQTY"], row);
                ...
                targetView.InvokeFieldUpdateService("FRealQty", row);
                ...
                {
                    targetView.Model.CopyEntryRowFollowCurrent("FInStockEntry", row, row, true);
                    targetView.Model.SetValue("FRealQty", srcOutData[i]["FREALQTY"], row + 1);
                    ...
                    //其他字段赋值
                    targetView.Model.SetValue("FSRCBILLTYPEID", targetView.Model.GetValue("FSRCBILLTYPEID", row), row + 1);
                    targetView.Model.SetValue("FSRCBillNo", targetView.Model.GetValue("FSRCBillNo", row), row + 1);
                    targetView.Model.SetValue("FPOOrderNo", targetView.Model.GetValue("FPOOrderNo", row), row + 1);
                    targetView.Model.SetValue("FSRCRowId", targetView.Model.GetValue("FSRCRowId", row), row + 1);
                    ...
                    targetView.InvokeFieldUpdateService("FRealQty", row + 1);
