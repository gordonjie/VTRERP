...
{
    [Description("扩展-库存盘点差异报告服务端插件")]
    public class JNStkCountDiffRpt : Kingdee.K3.SCM.App.Stock.Report.StkCountDiffRpt
    {
        ...
        public override Kingdee.BOS.Core.Report.ReportHeader GetReportHeaders(Kingdee.BOS.Core.Report.IRptParams filter)
        {
            ...
            reportHeader.AddChild("FJNTonProperty", new LocaleValue("标吨_账存", base.Context.UserLocale.LCID));
            return reportHeader;
        }
        ...
        public override void BuilderReportSqlAndTempTable(Kingdee.BOS.Core.Report.IRptParams filter, string tableName)
        {
            ...
            string strSql = string.Format(@"/*dialect*/select t1.*,case when ISNULL(t1.FBaseAccountQty,0)<>0 and t2.FIsMeasure='1'  then CONVERT(varchar(50),CONVERT(float,CAST(t1.FSecAccountQty/t1.FBaseAccountQty as decimal(18,4)))) else '' end FJNUnitEnzyme,
                                          case when ISNULL(t2.FJNTonProperty,0)<>0  and t2.FIsMeasure='1' then CONVERT(varchar(50),CONVERT(float,CAST(t1.FSecAccountQty/(t2.FJNTonProperty*1000) as decimal(18,4)))) else '' end FJNTonProperty
                                          into {0} from {1} as t1
