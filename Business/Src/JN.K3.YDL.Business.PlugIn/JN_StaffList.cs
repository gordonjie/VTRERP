...
{
    [Description("员工-列表插件")]
    public class JN_Stafflist : AbstractListPlugIn
    {
...
(select t1.Fpostid,t1.FREPORTTYPE,t3.FID,t3.FNUMBER,FSUPERIORPOST,lvl=1 from T_ORG_POSTREPORTLINE t1
...
select a.Fpostid,a.FREPORTTYPE,f.FID,f.FNUMBER,a.FSUPERIORPOST,lvl=b.lvl+1 from T_ORG_POSTREPORTLINE a 
...
update 
T_BD_STAFF  set F_JNREPORT1=(
select top 1 职位 from(
select 职位=case lvl when 1 then '1级' when 2 then '2级' when 3 then '3级' when 4 then '4级' else '员工' end,FNUMBER,FREPORTTYPE from cte) c 
 where T_BD_STAFF.FNUMBER=FNUMBER
