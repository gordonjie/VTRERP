...
{
    [Description("销售预测单-表单插件")]
    public class JN_YDL_SAL_ForecastEdit:AbstractBillPlugIn
    {
...
and t1.FUSEORGID={1}", Materialid, SupplyOrgid);
        ...
        public override void BeforeSave(BeforeSaveEventArgs e)
        {
            ...
            DynamicObjectCollection entityRows = this.View.Model.DataObject["SAL_ForecastEntity"] as DynamicObjectCollection;
        ...
        private DateTime getendsubdate()
        {
            ...
            if (date > 25)
            {
                compareDate = new DateTime(now.AddMonths(3).Year, now.AddMonths(3).Month, 1).AddDays(-1);
            }
            if (date <= 25)
            {
